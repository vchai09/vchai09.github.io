...
knitr::opts_chunk$set(fig.width=10, fig.height=8, warning=FALSE, message=FALSE, cache=FALSE, echo=FALSE)
...
# NOTE: When producing a SharedData$new object, it needs to have a unique name,
#  or it will utilize the single filter for every figure that uses it so we need 
#  a unique name based on the var
...
r_var3_sd <- SharedData$new(r_var3, ~State, 
                            group = paste0("Choose state(s) for ",unique(r_var$name)," values:"))
...
<!-- r_var3_sd <- SharedData$new(r_var3, ~State, group = paste0("Choose state(s) for ",unique(r_var$name)," values:")) -->
